...
def ocr_space_file_api(file_content, api_key, language='eng'):
    ...
    # Set a default filename with extension, such as 'uploaded_image.png'
    files = {'file': ('uploaded_image.png', file_content, 'image/png')}
...
async def process_file(file: UploadFile):
    ...
    # Process Image files using OCR.space API
    elif file.content_type in ['image/png', 'image/jpeg', 'image/jpg', 'image/gif']:
        try:
            content = await file.read()
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error processing image with OCR.space API.")
    else:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Unsupported file type.")
